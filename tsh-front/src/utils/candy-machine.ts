...
import { COLLECTION_ADDRESS} from "../constant/contract";
  ...
  for (let index = 0; index < tokenAccounts.value.length; index++) {
    try {
      ...
      if (tokenAmount.amount == "1" && tokenAmount.decimals == "0") {
        ...
        if (metadata.data.creators[0].address == COLLECTION_ADDRESS) {
...
export async function getNftsForOwner1(
  ...
  const allTokens: any = []
    ...
    for(let i = 0; i < nftArray.length; i++) {
      let nft = nftArray[i];
      console.log(nft.data.creators[0].address + " -- " + i);
      
      if (nft.data.creators[0].address == COLLECTION_ADDRESS) {
        try{
